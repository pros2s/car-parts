...
class CartFull extends Component {
  ...
  getTotalPrice() {
    ...
    var total = 0;
    this.props.basketproducts.map(
      (e) => (total = total + parseInt(e.product.price, 10) * e.quantity)
    );
    return total;
  }
      ...
      <div key={item.product._id} className='row m-lg-0 overl bor-r'>
        ...
        <div id='containerqtparent' className='col-sm-2 col-md-2 col-lg-2 cart-item '>
          <div className='containerqt'>
            ...
            <span id={JSON.stringify(item)} type='number' min='1' className='qt'>
              {item.quantity}
            </span>
            <span
              id={JSON.stringify(item)}
        ...
        <div className='col-sm-1 col-md-1 col-lg-1 cart-item'>
          <i
            ...
            onClick={() => this.props.removeFromBasket(item.product._id)}
          />
        </div>
      </div>
    );
...
export default connect(mapStateToProps, { removeFromBasket, addToBasket, changeBasketItem })(
  CartFull
);
